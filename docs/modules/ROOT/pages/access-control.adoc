...
AppGara provides xref:api:access.adoc#Ownable[`Ownable`] for implementing ownership in your contracts.
...
Shared AppGara Contracts provides xref:api:access.adoc#AccessControl[`AccessControl`] for implementing role-based access control. Its usage is straightforward: for each role that you want to define,
