...
[.readme-notice]
NOTE: This document is better viewed at https://docs.appgarra.com/contracts/api/proxy
...
CAUTION: Using upgradeable proxies correctly and securely is a difficult task that requires deep knowledge of the proxy pattern, Solidity, and the EVM. Unless you want a lot of low level control, we recommend using the xref:upgrades-plugins::index.adoc[AppGara Upgrades Plugins] for Truffle and Hardhat.
